...
public class UlidTests(ITestOutputHelper output) : CoerceTests(output)
{
    ...
    [
        ...
        new GenericCoerceTest<Ulid, PrimaryColor>{ Name = $"Ulid To Enum ({TestUlidString})", Input = TestUlid, ExpectedResult = false },
    ...
    [
        ...
        new NonGenericCoerceTest<Ulid, PrimaryColor>{ Name = $"Ulid To Enum ({TestUlidString})", Input = TestUlid, ExpectedResult = false },
